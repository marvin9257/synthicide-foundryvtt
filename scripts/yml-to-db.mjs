...
async function main() {
  ...
  for (const dir of allDirs) {
    ...
    if ((await fs.stat(fullPath)).isDirectory()) {
      console.log('[TRACE] yml-to-db.mjs: checking if dir startsWith .', { dir });
      if (!dir.startsWith('.')) {
        console.log('[TRACE] yml-to-db.mjs: dir does NOT startWith .', { dir });
      } else {
        console.log('[TRACE] yml-to-db.mjs: dir startsWith .', { dir });
      }
      packDirs.push(dir);
    }
  }
