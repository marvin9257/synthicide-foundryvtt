...
/**
 * Custom token ruler for measuring in virtual 3x3 grid units (vgu).
 * Extends Foundry's TokenRuler to display vgu and cost for token movement.
 */
export default class SynthicideVirtualTokenRuler extends foundry.canvas.placeables.tokens.TokenRuler {
  /**
   * Get the label context for a waypoint, displaying vgu and cost if enabled.
  ...
  _getWaypointLabelContext(waypoint, state) {
    ...
    if (game.settings.get('synthicide', SYNTHICIDE.VIRTUAL_GRID_MOVEMENT_KEY) && game.combats?.active?.started && waypoint.previous) {
      ...
      // Total vgu from origin to this waypoint
      let total = 0;
      let prev = waypoint;
