...
- [module/data/base-item.mjs](module/data/base-item.mjs)
...
```mermaid
sequenceDiagram
	participant I as Item Hook (_onCreate/_onUpdate/_onDelete)
	participant B as SynthicideItemBase
	participant A as SynthicideActor

	I->>B: triggerActorModifierAggregation()
	B->>A: scheduleModifierAggregation({debug})

	alt first call in current tick
		A->>A: create shared Promise
		A->>A: queue setTimeout(..., 0)
	else already queued
		A->>A: return existing Promise
	end

	A->>A: aggregateAndApplyItemModifiers()
	A->>A: update changed system.attributes.*.modifier
	A->>A: recompute current values in memory
	A->>A: apply non-attribute modifiers
	A-->>B: resolve shared Promise
```

### How to read this

1. Item changes trigger `triggerActorModifierAggregation()`.
2. The actor queues exactly one aggregation run per event-loop turn.
3. Multiple callers before execution share one Promise and do not create duplicate runs.
4. If any caller sets debug true, the queued run executes with debug enabled.
5. The queued run performs one aggregate/apply pass and clears queue state.

### Why this exists

- Prevents update storms when many item hooks fire together.
- Keeps actor modifier computation deterministic for a single burst.
- Preserves call-site ergonomics (callers can await the same Promise).
